...
**Feature ID**: W-FA1(per [`DESIGN-W-DEPLOYMENT`](../INTEGRATION-DESIGN-W-DEPLOYMENT.md) §11 line 1114 — Phase W-7 Track DESIGN-A 專屬、deploy 階段第一個 A-track feature)
**Feature Branch**: TBD(spec-kit `/speckit-specify` 階段建立,預期 `014-compose-nestjs-service`)
**Created**: 2026-05-18
...
- [`docs/INTEGRATION-DESIGN-W-DEPLOYMENT.md`](../INTEGRATION-DESIGN-W-DEPLOYMENT.md) §3.4-A(nestjs service yaml 草稿)、§11 line 1114(W-FA1 scope 描述)、§11.5(profile 啟動命令)
...
- 既有 [`docker-compose.dev.yml`](../../docker-compose.dev.yml)(W-F7 落地、W-FA1 加 nestjs host port 條目)
- 既有 [`docker-compose.prod.yml`](../../docker-compose.prod.yml)(W-F6 落地、W-FA1 加 nestjs prod override 條目)
- 既有 [`deploy/secrets/`](../../deploy/secrets/)(W-F4 _FILE pattern 結構、W-FA1 加 1 個新 secret + 沿用 4 個既有 secret)

**Scope summary**:rev1 deploy 階段 Track DESIGN-A 專屬第一個 feature(P7 Track-A 三件套 W-FA1/W-FA2/W-FA3 中最先動)。**把 nestjs service 加進 rev1 docker compose stack、走 profile=track-a 啟動模式、共享 rev1 既有 postgres + redis、JWT secret 透過 W-F4 _FILE pattern 與 rust-api 共享**。範疇刻意收緊到「compose 層 + nestjs container 起得來 + 能 connect rev1 backing services + sys_tokens schema acceptance verify」、不含 application code 改動(refreshToken 實作是 F10 範疇)、不含 nginx routing(W-FA2 範疇)、不含 Casbin pub-sub channel 訂閱(F10 範疇)。

**Commit 模式**:**單段 commit**(per W-F1~W-F7 慣例)— 只動 outer repo,**不動 nestjs fork repo**(fork260509-soybean-admin-nestjs 不 worktree 化,屬 transitional 不投資 worktree+submodule overhead)。

**範疇外**:nestjs application source code 改動 / sys_tokens prisma model 對齊 / refreshToken endpoint 實作 / nginx track-a.inc upstream routing / Casbin pub-sub channel `casbin:policy:invalidate` 訂閱 / nestjs fork repo worktree 化。
...
- **Q1**: W-FA1 scope 要劃到哪一層? → **A: B 中度**(nestjs container 起得來 healthy + JWT secret 共享 `_FILE` pattern + DB 共享 rev1 postgres + sys_tokens schema acceptance verify + healthcheck)。對齊 DESIGN-W §11 line 1114 「JWT secret 共享 + sys_tokens schema 對齊驗證」描述;**不含** F10 application 邏輯(那是獨立 feature)。
...
1. **Given** dev stack 已啟、`docker-compose.yml` / `docker-compose.dev.yml` 含 W-FA1 nestjs service 條目、nestjs image 已 build(`nestjs:rev1-admin-nestjs`),**When** `docker compose -f docker-compose.yml -f docker-compose.dev.yml --profile track-a up -d --wait`,**Then** `docker compose ps` 顯示 nestjs 為 `(healthy)` 狀態、6 個 既有 service 仍全 healthy
...
對齊 W-F4 secret-injection 紀律(Principle 全 stack 一致)、避免 plaintext password 洩 docker config / `docker inspect` / image layer。

**Acceptance Scenarios**:

1. **Given** stack 已啟,**When** `docker compose exec nestjs sh -c 'env | grep -E "JWT_SECRET=|DATABASE_URL=|REDIS_PASSWORD="' | wc -l`,**Then** 3 個 env 都已設(`>= 3`)、且 `docker compose config` 完全不洩 plaintext secret 值
2. **Given** stack 已啟,**When** `docker compose exec nestjs sh -c 'echo "JWT_SECRET value length: $(echo -n $JWT_SECRET | wc -c)"'`,**Then** length = 64(對齊 `deploy/secrets/jwt_secret.txt` 64 chars hex)
...
1. **Given** stack 已啟,**When** `docker compose exec postgres psql -U soybean -d soybean_admin_rust -c "\d sys_tokens"`(從 host 跑、不從 nestjs container 跑),**Then** sys_tokens 表 schema 顯示(若 rust-api migration 未落 sys_tokens schema、W-FA1 acceptance 跳過此驗、改為 follow-up after F10 schema migration)
2. **Given** stack 已啟,**When** `docker compose exec nestjs sh -c 'apk add --no-cache postgresql-client && psql "$DATABASE_URL" -c "SELECT 1;"'`(若 nestjs alpine image 含 psql 或臨時 install),**Then** 回 `1`(connect 成功)— 此 step 為可選驗(不強制 alpine apk add、屬 debug 階段)、acceptance 主要由 nestjs healthcheck 隱含驗 connect
3. **Given** stack 已啟,**When** `docker compose logs nestjs --tail 30`,**Then** 無 `DATABASE_URL` connection error / `ENOTFOUND postgres` / `auth failed for user soybean` 等錯
...
2. **Given** DESIGN-B 啟動,**When** `docker compose -f docker-compose.yml -f docker-compose.dev.yml ps --services` & grep nestjs,**Then** 結果為空 / exit 1

---

### User Story 6 — base-web / rust-api / 既有 W-F* 零回歸(Priority: P2)

**Goal**:W-FA1 加 nestjs service 不影響既有 6 service 行為、F6 / F5.1 既有 browser login 流程仍 PASS、base-web src / rust-api src 零改動。
...
2. **Given** W-FA1 changes only(對齊 spec FR-018),**When** `git diff HEAD~1 -- base-web/src/ rust-api/server/ rust-api/migration/`,**Then** 無輸出
...
| E-2 | `jwt_secret.txt` 不存在 | nestjs container 啟動 fail(secret required、無 fallback);docker compose 顯示 error;W-FA1 acceptance prereq T005 已備齊 5 secret 含可選 6th refresh_token_secret |
...
- **FR-002**: nestjs service `image` 指 `nestjs:${NESTJS_IMAGE_TAG:-rev1-admin-nestjs}`、build context 為 `fork260509-soybean-admin-nestjs/backend/`、Dockerfile 為 fork 既有 `backend/Dockerfile`(Q2)。
- **FR-003**: nestjs container `entrypoint` MUST 用 `sh -c` wrapper 從 `_FILE` secret bridge 成 env var(per Q2 不動 nestjs source 自然推論);必含 4 個 export:`JWT_SECRET` / `REFRESH_TOKEN_SECRET`(可 fallback JWT_SECRET)/ `DATABASE_URL` / `REDIS_PASSWORD`,exec `node dist/apps/base-system/src/main`(對齊 nestjs fork package.json `start:prod` 啟動點)。
...
- **FR-006**: `docker-compose.yml` `secrets:` section MUST 加 `refresh_token_secret:` 條目對齊 W-F4 既有 5 個 secret 格式。
...
- **FR-012**: `docker-compose.prod.yml` MUST 加 nestjs service override(目前範疇:無 ports;留 W-FA2 落地時加 internal routing、不從 host 暴露)。實際 prod override 為空 service block 或最簡(對齊 W-F6 prod.yml 結構)。
- **FR-013**: `CLAUDE.md` §5.2 / §5.2.1 MUST 更新 dev/prod 啟動命令範例加 track-a profile 變體(對齊 W-F6 / W-F7 multi-mode 啟動文件慣例)。
- **FR-014**: `docs/INTEGRATION-CHECKLIST.md` MUST 加 W-FA1 row 進 Phase W roadmap 表(若已有 W-FA row、加 ✅;若無、新增)+ Current Focus 更新 + 已完成里程碑加 W-FA1 條目。
...
- **FR-018**: nestjs fork source(`fork260509-soybean-admin-nestjs/`)任何檔案 MUST 零改動(per Constitution Principle IV + Q2);base-web `src/` 任何檔案 MUST 零改動;rust-api worktree 任何檔案 MUST 零改動。
- **FR-019**: W-FA1 MUST 不跑 nestjs 自帶 prisma migrate / db seed(per Q4)— compose 不加 nestjs db-init service、nestjs runtime 不 invoke prisma migrate;sys_tokens schema 對齊驗交給 acceptance 階段 manual SQL(per FR-021 acceptance)。
...
- **SC-006**: F6 browser login acceptance(via CDP)在 W-FA1 落地後仍 PASS(F6 + F5.1 不受 W-FA1 影響、SC-006 = regression verify)。
...
- **OQ-1**: nestjs container 內 prisma client lazy-init(runtime first DB query)時若發現 schema mismatch(sys_tokens prisma model 不對齊 rust-api migration 落的真實 schema)是否會 crash?若 crash、W-FA1 healthcheck `/v1/route/getConstantRoutes` 是否仍 PASS(該 endpoint 可能不 touch sys_tokens)?屬 plan 階段 grep nestjs source `apps/base-system/src/modules/auth/` 確認、若 crash 風險高、W-FA1 範疇調整(可能需臨時 stub sys_tokens prisma model)。
- **OQ-2**: nestjs alpine image 是否含 `psql` client(US3 scenario 2 可選驗 from inside container)?若無、acceptance scenario 改為 from host 透過 docker compose exec postgres 跑。屬 acceptance 階段 grep 即知、不阻 W-FA1 plan。
- **OQ-3**: nestjs `model.conf`(Casbin)在 fork backend Dockerfile build 時是否 COPY 進 image 正確路徑、runtime CASBIN_MODEL env 是否能找到?屬 plan 階段 grep `apps/base-system/src/lib/` + `model.conf` 在 fork repo 內位置確認、若有 build context issue、W-FA1 範疇加 1 個 mount volume override(`-v ./model.conf:/app/model.conf`)。
- **OQ-4**: nestjs container `restart: unless-stopped` 在 `--profile track-a` down 後是否會被 docker daemon 重啟?(profile-based service 是否有特殊 restart 行為?)屬 plan 階段 docker compose docs 確認、若需特殊處理、W-FA1 範疇處理。

---

**Brainstorming 完成、`/speckit-specify` 啟動條件滿足**。預期 spec-kit 流程產出在 `specs/014-compose-nestjs-service/`、4-5 個 P1/P2/P3 acceptance dimension、~10-15 個檔案改動、~25-35 個 task、~60-90 分鐘實作。
