...
**Feature ID**: F4（per [`DESIGN-A`](../INTEGRATION-DESIGN-A-RUST-NESTJS.md) §6.1）
...
- [`docs/INTEGRATION-DESIGN-A-RUST-NESTJS.md`](../INTEGRATION-DESIGN-A-RUST-NESTJS.md) §3.1 / §4.1 / §6.1 F4
- [`docs/INTEGRATION-DESIGN-B-RUST-ONLY.md`](../INTEGRATION-DESIGN-B-RUST-ONLY.md) §4.1（同 DESIGN-A）
- [`.specify/memory/constitution.md`](../../.specify/memory/constitution.md) v1.0.0（Principle II audit + Principle IV base 不改動 + 架構約束）

**Input**：rust admin 全 API 表面 response shape 對齊 base 預期 — 一次性解決 RESEARCH GAP B1 + B2 + B3 + B5。

---

## User Scenarios & Testing *(mandatory)*

### User Story 1 — rust admin response shape 全面對齊 base 預期（Priority: P1，唯一 US）🎯 MVP
...
**Why this is the only US (P1，no further decomposition)**：

F4 的 3 個 GAP 群（B1+B2 code / B3 camelCase / B5 欄位補齊）**並非獨立可交付**：
- 單獨修 B1+B2 → base 能判斷成敗，但 `data.refresh_token` undefined（TS 期望 `refreshToken`）→ login flow 仍 broken
- 單獨修 B3 → 欄位 camelCase 對了，但 code=200 ≠ 0000 → base 永遠 onBackendFail → 整個 flow 不通
...
**Independent Test**：base + rust + postgres + redis 起來，跑「Soybean / 123456 登入」 → 預期 base 進入 home view 且 console 無 onBackendFail 警告（涵蓋 dimension A + B + C 端到端）。

#### Acceptance Scenarios — Dimension A：Response code 路線 II 落地（GAP B1 + B2）

1. **Given** base `.env` 設 `VITE_SERVICE_SUCCESS_CODE=0`，**When** base 收到 rust HTTP 200 + body `{code: 0, data: {...}, msg: "success", success: true}`，**Then** base `onBackendSuccess` 觸發、data 進入 view layer
...
1. **Given** rust struct 含 `refresh_token: String`，**When** serialize，**Then** JSON 欄位為 `refreshToken`（無 `refresh_token`）
2. **Given** rust struct 含 nested struct（如 `UserRoute { routes: Vec<MenuRoute>, home }`、`MenuRoute` 內含 `route_name` 等），**When** serialize，**Then** 各層級欄位都 camelCase（`routes`/`home`/`routeName`/...）
3. **Given** 既有 `UserInfoOutput` 含 per-field `#[serde(rename = "userId")]`，**When** 套用 F4 後改為 struct-level `#[serde(rename_all = "camelCase")]`，**Then** JSON 輸出與舊版完全等價（`user_id` → `userId`）

#### Acceptance Scenarios — Dimension C：Auth / UserInfo 欄位補齊（GAP B5）

1. **Given** rust `/auth/login` response，**When** base parse，**Then** `data` 物件含 `token: string` 與 `refreshToken: string` 兩欄位（透過 Dimension B 的 camelCase 達成、無需獨立改 code）
2. **Given** rust `/auth/getUserInfo` response，**When** base parse，**Then** `data` 物件含 `userId, userName, roles, buttons` 4 個欄位（齊全、無多無少、`roles` 與 `buttons` 為 array 型別）
3. **Given** F4 階段不涉及 button-level RBAC 配置，**When** user 取 getUserInfo，**Then** `buttons: []`（空陣列、欄位存在）；具體 button RBAC 內容來源（per-role config / menu-derived）留 **F7 manage-crud-alignment** 或更後續 feature

### Edge Cases

- **空集合 vs null**：rust `Vec<T>` 預設 serialize 為 `[]`；`Option<Vec<T>>::None` serialize 為 `null` — 對 base TS `string[]` 期望，所有 collection 欄位必須用 `Vec<T>` 而非 `Option<Vec<T>>`（統一為 `[]`）
- **Pagination wrapper**：`PaginatedData<T>` 也是 Res 的 `data` 內容、必須 rename_all camelCase；分頁欄位（page / pageSize / total / records）需對齊 base 期望
- **Date/time 欄位**：`serde_json` 預設 ISO 8601 字串；欄位名透過 rename_all 從 `created_at` 變 `createdAt`（值格式不變）
...
- **Middleware-level errors**：Casbin deny / JWT 過期 / API key 無效 — 都必須經 `IntoResponse` 轉成 Res 形式（HTTP 200 + body code），不可裸 HTTP 401/403
...
- **Nested struct rename**：rename_all **不遞迴** — 每個 struct（含 nested）都必須有自己的 rename_all attribute
- **Sea-ORM entity 直接 return**：rust handler 若直接 return DB entity struct（非 DTO wrapper），該 entity 也須加 rename_all（spec-kit `/speckit-plan` 階段 audit 列出）
...
- **FR-002**: 所有 admin handler response MUST 用 `Res<T>` envelope，含 `code: u16`、`data: Option<T>`、`msg: String`、`success: bool` 四欄位
...
- **FR-005**: 所有 error response code 值 MUST 來自下方 §Key Entities「Business code namespace」表（含 base 既有 8 條 + rust 新增 15 條）— 禁 magic number
...
- **FR-018**: 新增 rust business code constants MUST 定義在單一共用模組（如 `server_core::web::code` 或類似 path），其他 handler 透過引用該模組常數使用
...
- **FR-011**: 既有 per-field `#[serde(rename = "...")]` directive MUST 移除，由 struct-level `rename_all` 取代（在輸出 JSON 等價的前提下；不等價的例外須在 spec-kit `/speckit-plan` 階段個別合理化）
- **FR-012**: 所有 collection 欄位 MUST 用 `Vec<T>` 而非 `Option<Vec<T>>`（空 vs missing 統一輸出 `[]`，不為 `null`）
- **FR-013**: Sea-ORM entity struct 若**直接**被 handler return（非透過 DTO wrapper），MUST 加 `rename_all`；spec-kit `/speckit-plan` 階段須 audit 列出此類 entity
...
- **FR-021**: rust middleware 層產生的錯誤（Casbin deny / JWT 無效 / API key 缺失）MUST 經 `IntoResponse` 轉為 `Res<T>` 形式（HTTP 200 + body 對應 code）；禁裸 HTTP 401 / 403
...
- rust 新增 codes 在 spec-kit `/speckit-plan` 階段 MUST 產出常數定義（避免 magic number — FR-018）
...
rust 新增的單一共用模組（例如 `server_core::web::code` 或類似 path）— 集中定義 4001 / 4002 / ... / 9005 等常數。具體模組路徑由 spec-kit `/speckit-plan` 階段拍板。
...
- **例外**：per-field `#[serde(rename = "...")]` 僅限**確有必要**（如欄位名不符合 camelCase 換 PascalCase / kebab-case 等特殊需求），且需在 spec-kit `/speckit-plan` 階段個別合理化
...
- **SC-001**: 100% rust admin endpoint 在正常 handler 路徑 return HTTP 200（隨機抽 5 個 endpoint curl 驗證、全 200）
- **SC-002**: base `_builtin/login` flow round-trip 跑通 — base 啟動 → 輸入 `Soybean / 123456` → rust 回 `{code:0, data:{token, refreshToken}, ...}` → base 進入 home view（無 onBackendFail console 警告）
- **SC-003**: `/auth/getUserInfo` response 結構 100% 對齊 base `Api.Auth.UserInfo` TS interface — curl JSON 對照欄位數 = 4、欄位名一致（`userId, userName, roles, buttons`）、`roles` 與 `buttons` 為 array 型別
...
- **base `.env` 可被修改**（per [`DESIGN-A`](../INTEGRATION-DESIGN-A-RUST-NESTJS.md) §1.3「可動：.env」原則 + Constitution v1.0.0 Principle IV）
- **`buttons` 欄位內容來源**（per-role config / menu-derived / hardcoded）為 **F7+ feature 範圍**；F4 只負責欄位存在且預設空陣列
- **Sea-ORM entity struct 直接被 handler return 的情況存在但有限**（spec-kit `/speckit-plan` 階段 audit 數量）— 不直接 return 的 entity 不在 F4 範圍
...
- **F4 與 F1 / F2 / F3 可平行進行**（無依賴關係，per [`DESIGN-A`](../INTEGRATION-DESIGN-A-RUST-NESTJS.md) §6.2）

---

> **下一步**：spec 完成後，依 brainstorming flow 進入 [`superpowers:writing-plans`](../../../home/anew/.claude/plugins/cache/claude-plugins-official/superpowers/5.1.0/skills/writing-plans/SKILL.md) skill 撰寫 implementation plan；具體 code patch / migration / test 由 spec-kit `/speckit-specify`（接手本檔轉為正式 spec.md）→ `/speckit-plan` → `/speckit-tasks` → `/speckit-implement` 流程落地。
