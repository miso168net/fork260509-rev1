...
- [`docs/superpowers/014-feature-compose-nestjs-service.md`](014-feature-compose-nestjs-service.md) W-FA1(落定 nestjs container port = 9528、GLOBAL_PREFIX = /v1、與 DESIGN-W §4.3 草稿 port 3000 + /auth/refreshToken 漏 prefix 兩處不一致、本 feature 對齊現實)
- [`docs/superpowers/013-feature-route-guard.md`](013-feature-route-guard.md) F6(F5.1 + F6 既有 SPA → nginx → rust-api 路由模式不變、W-FA2 只加 1 條新 exact-match location 不影響既有)
