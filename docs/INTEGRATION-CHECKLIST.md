...
**現狀**：DESIGN-A §6.1 全 14 application feature（F1–F14）+ Phase W deploy 全部 7 階段（W-F1~W-F7、W-F11、Track DESIGN-A W-FA1/2/3 + **W-F12/13/14 observability**）全部落地，**DESIGN-B（rust-only）形態生效**。**W-WEBUI 軌道（base-web 管理後台接線）已完成** —— 031 user-crud、032 menu-crud、033 role-crud、034 role-authorization 四 feature 全部落地。**W-WEBUI follow-up 軌道：W-FW5/6/7/8 + W-FW9 wire-id-consistency 全部落地** —— 035 user-role-and-password-wiring + 036 menu-field-persistence + 037 role-authorization-completion + 038 button-auth-completion + 040 wire-id-consistency。**044 observability-and-cleanup-pass 落地** —— W-F12 (promtail+Loki+rust JSON tracing) + W-F13 (8 業務 metric + prometheus + 3 exporter) + W-F14 (grafana 4-6 dashboards + ≥6 alert rules、built-in unified alerting) + P2 4 cleanup (042-N2/N6 + F3-N4 + 041-N1 errata)；軌道外 rust-api + outer、0 base-web、0 schema migration；Phase W deploy P5 收尾。

**Active feature**：—（044 observability-and-cleanup-pass 已完成、見已完成里程碑）
...
| 042-N1 | 042 implementer concern | 4 個 `#[ignore]` integration test（`server/model/tests/{audit_http_middleware,audit_basics,audit_transaction_rollback,soft_delete_audit_integration}.rs`）以 `write_in_txn` 後立刻 query `sys_operation_log` 為 assertion 模式；042 outbox refactor 後 row 先進 sys_audit_outbox、drainer 異步消化才到 sys_operation_log → 跑 `cargo test --ignored` 會 fail | 不影響 compile / 正常 `cargo test`、不阻塞 042 acceptance；下次觸碰這些 test 時調整：（a）測試內 sleep 等 drainer、（b）改 query sys_audit_outbox、（c）or in-test spawn drainer + wait |
...
| 044-N1 | 044 implementer 發現 | `sys_authorization_service.rs` 3 處 production `println!`（lines 140/154/168 in `assign_permission` 內、`existing_permissions`/`new_policies`/`existing_policies` debug 印）—— research.md R-3 「1 處 print!」漏抓；W-F12 後會 bypass JSON formatter 產 non-JSON garbage row 落 Loki | 任一後續 feature 順手清 3 處 `println!` → `tracing::debug!(?…, "…")` 或刪除；non-blocking、屬 spec rot |

> ⚠️ F3-N1~N3 為 F3 階段（2026-05-14）所留、迄今未正式 review 結案 —— 下次觸及 audit / endpoint facade / `sys_access_key` 區域時應逐項查證並結案；F3-N4 已於 044 結案（`sys_user_api.rs:40` 1 處 `print!` 清掉、其他 `println!` 在 cfg(test) 或 startup CLI 範圍合理保留、新發現 044-N1 為 production `println!` 3 處待清）。
