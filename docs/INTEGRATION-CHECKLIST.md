...
**Active feature**：—（043 spec-hygiene-pass-2 已完成、見已完成里程碑）

**下一步**：W-F12/13/14 observability（Phase W deploy P5）—— 042 已開放 `audit:events` Redis Stream + sys_audit_outbox 表 + sys_operation_log 雙視角，subscriber 即可消費 audit 事件。
...
| 041-N1 | 041 implementer 發現 | 030 C-V8 / 022 C-V3a `status` enum 後續可能再演進（W-FW5/F8 後 status 在某些路徑為 string `"1"`/`"2"` 但 sys_user / sys_role 的 DB `status` 仍 `enabled`/`disabled`）；混雜模式可能在後續 feature 撞坑、未進 041 scope（per FR-011） | spec hygiene-pass-2 候選；待下一次 regression 確認影響面 |
...
| 042-N2 | 042 code-quality reviewer | `extract_entity_id_from_url` 對 `/systemManage/<verb>/<id>` paths 抽錯（e.g. `/systemManage/deleteMenu/123` → 拿 `deleteMenu` 不是 `123`）；fn nth(2) 只對 native admin `/role/<id>` 正確、systemManage path 需 nth(3) | sys_operation_log.entity_id 對 systemManage write 路徑值為 verb；audit forensic 仍可從 `url` / `entity_type` 推、`entity_id` 為輔；任一觸碰 audit url parsing 之 feature 順手修（hybrid rule：若 path 第 1 segment 為 `systemManage` 則 nth(3)、否則 nth(2)）|
| 042-N4 | 042 C-V12 Part B | SC-005 stretch p50 ≤ 100ms 未達（p50=175ms with `drainer_sleep_interval_ms=50`、p95=188ms）；spec C-V12 預測 100-200ms 落點正常、p95 ≤ 500ms PASS | 可選優化：（a）middleware → drainer channel notify 替代 polling、（b）publish_audit_event inline in drainer per-row txn、（c）lower fetch txn overhead；defer 至 W-F12/13/14 觀測需求驅動再決定 |
...
| 042-N6 | 042 implementer / T016 | `OperationLogContext.module_name` / `description` middleware 寫 `"TODO"` placeholder（pre-existing F2.1 留下、operation_log.rs:121-122）；042 path 不讀此 2 欄（從 url/operation 重新計算）但仍流入 outbox JSON → sys_operation_log row | sys_operation_log row 內 `module_name` / `description` 兩欄不準（前者實際從 entity_type 取代、後者 042 統一寫 `"HTTP {method} {url}"`）；reader 應從 `url` / `entity_type` 推導；任一觸碰 middleware 之 feature 順手清 placeholder |
