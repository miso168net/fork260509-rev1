...
**Active feature**：—（042 audit-outbox-and-http-mount 已完成、見已完成里程碑）
...
| 041-N2 | 041 implementer 發現 | 002 §E4 第二段範例 code block 內 use 行 `use server_model::admin::entities::{...}` 仍是 brainstorm 期推測（impl 在 model crate 內、實際應 `use crate::admin::entities::...`）；errata 041 已修 path comment、未動 use 行；未進 041 scope（per FR-011） | spec hygiene-pass-2 候選；本身 documentation 細節、不影響 reader 抓 impl 位置 |
...
| 042-N3 | 042 acceptance / CDP smoke | C-V verification SQL 用 `NOW() - INTERVAL '...'` 時間過濾、postgres `NOW()` 是 timezone-aware（local TZ e.g. `+08`）、`sys_operation_log.created_at` 是 `TIMESTAMP without time zone`（Rust `Utc::now().naive_utc()`）；兩者直接相減 8 小時誤差、過濾掉真實 row | 影響 042 verification-commands.md C-V4 / C-V9 等 time-window query；fix：`created_at > (NOW() AT TIME ZONE 'UTC')::timestamp - INTERVAL '...'`；spec hygiene-pass-2 候選 |
