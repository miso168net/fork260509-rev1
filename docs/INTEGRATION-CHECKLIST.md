...
**Phase**:application Phase 4 **整套收尾完成 + F11 extracted-stubs 收尾**(F10 wire-up + F10.1 R-8 修 + F10.2 R-7 修 三件套全完整、refreshToken end-to-end pass + F11 DESIGN-A §4.2 抽離項清單 4/5 條 stub endpoint 補位 + Casbin policy seed、base-web + nestjs fork **三邊零改動**);Phase W deploy P7 Track DESIGN-A 三件套全完成;application Phase 2 F5.1 + F6 完成
**Active feature**:F11 extracted-stubs(兩段式 commit pending push、rust-api worktree + outer 020-extracted-stubs 兩段就位)
**下一步**:F9 systemManage-alias-router(含 DESIGN-A §4.2 剩 1 條 batchDeleteUser stub);application 並行 F7 / F8 / F12 / W-F11 / W-F6b;F13 rust-refresh-token-impl + F14 DESIGN-A→B cutover 後續(DESIGN-B 階段、F11 已補完 4/5 抽離項 stub 解鎖 DESIGN-B rust 自驗起點)
...
| F11 | `extracted-stubs` | ✅ | ✅ | ✅ | ✅ | ✅ | **完成**(outer `<sha-pending>` + merge `<sha-pending>`、rust-api `c58e619`、7/7 acceptance PASS、4 條 stub + Casbin policy seed、DESIGN-A §4.2 抽離項 4/5 完成)|
| F12 | `cleanup-job` | — | — | — | — | — | 未啟(application Phase 4 同期、不直接依賴 F10、可並行)|
