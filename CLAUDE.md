...
<!-- SPECKIT START -->
- **Active feature**: —(無進行中 feature;032 menu-crud-wiring(W-WEBUI 軌道第二個 feature)落地完成、多段式 commit + push + merge `<sha-pending>` done)
- **Phase**: Done — 032 `032-menu-crud-wiring` 落地(outer `<sha-pending>` + merge `<sha-pending>`、rust-api `149dc52`、base-web `b43634c0`;W-FW2 W-WEBUI 軌道第二個 feature、base-web menu CRUD 接線;acceptance C-V1~C-V11 11/11 PASS;含 2 個 acceptance 階段 bug fix:① sys_menu id sequence desync — `m20260522_c_sys_menu_id_seq_fix` setval migration(spec FR-016 受核准例外、user 明示核准)② parentId 字串/數字契約落差 — `deserialize_with` helper 雙格式相容;subagent-driven-development + 兩段式 spec/code review + TDD 執行)。後續候選:W-FW2-N1 menu query/buttons/fixedIndexInTab 持久化、W-FW1-N1 userRoles 接線、W-FW1-N2 password UX、observability W-F12/W-F13/W-F14。
- **Previous features**: W-F1 merge `430ada9` / W-F2 merge `ac79ed0` / W-F3 merge `04671d0` / W-F4 merge `ab658d7` / W-F5 merge `dff14c2` / W-F7 merge `62b3475` / W-F6 merge `5e38030` / F6 merge `a431215` / W-FA1 merge `b095d55` / W-FA2 merge `c5b7840` / W-FA3 merge `f23f38e` / F10 merge `8f0e84c` / F10.1 merge `48b70e6` / F10.2 merge `851ec79` / F11 merge `81ecb0d` / F9 merge `b2f910c` / F7 merge `136b1eb` / F7.1 merge `efe910e` / F7.2 merge `476ca88` / F8 merge `c2b0912` / W-F11 merge `d2d4c4c` / F12 merge `86e56e5` / F13 merge `4e9cf07` / F14 merge `1f20a0d` / 030 merge `0ed2e85` / 031 merge `a09d316` / 032 merge `<sha-pending>`(均已 push、acceptance PASS;F13 為 DESIGN-A §6.1 Phase 5 P5 第一個 feature — rust 補實作 POST /auth/refreshToken refresh token 輪替;F14 為 P5 第二個也是最後一個 feature — nestjs 退場、DESIGN-B(rust-only)形態正式生效、DESIGN-A §6.1 全 14 feature 收尾;031 為 W-WEBUI 軌道第一個 feature — base-web user CRUD 接線、Constitution v1.1.0「W-WEBUI 受管例外」首次行使、11/11 C-V acceptance PASS;032 為 W-WEBUI 軌道第二個 feature — base-web menu CRUD 接線、Constitution v1.1.0「W-WEBUI 受管例外」第二次行使、11/11 C-V acceptance PASS、含 sys_menu id sequence desync + parentId 字串/數字落差 2 個 bug fix)
<!-- SPECKIT END -->
