...
<!-- SPECKIT START -->
- **Active feature**: `034` `role-authorization-wiring`(W-FW4、W-WEBUI 軌道第四個也是最後一個 feature)—— spec-kit 設計鏈進行中:specify / clarify / plan 完成,plan `specs/034-role-authorization-wiring/plan.md`;待 `/speckit-tasks`。
- **Phase**: 設計中 — brainstorm(`docs/superpowers/034-feature-role-authorization-wiring.md`、outer `e75e63d`)+ spec(`specs/034-role-authorization-wiring/spec.md`、16 項 quality checklist 全通過、0 NEEDS CLARIFICATION)+ clarify(掃描無歧義、未改 spec)+ plan(Constitution Check 5/5 PASS、Phase 0 research R-Q1~R-Q4 resolved)。拍板「只做菜單授權」:rust E1–E5 補 `/systemManage/{getRoleMenuIds,assignRoleMenus}` 2 alias(getRoleMenuIds 讀 / assignRoleMenus 寫、domain 伺服器端注入)+ SysAuthorizationService Extension wiring + Casbin seed migration;base-web 2 檔接 `menu-auth-modal.vue` 的 getChecks/handleSubmit。button-auth / role home 為範疇外 follow-up W-FW4-N1 / W-FW4-N2。比照 W-FW1/W-FW2/W-FW3。
