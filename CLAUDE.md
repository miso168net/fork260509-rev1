...
<!-- SPECKIT START -->
- **Active feature**: `035` `user-role-and-password-wiring`(W-FW5、W-WEBUI follow-up 軌道第一個 feature)—— spec-kit 設計鏈進行中:specify / clarify / plan 完成,plan `specs/035-user-role-and-password-wiring/plan.md`;待 `/speckit-tasks`。
- **Phase**: 設計中 — brainstorm(`docs/superpowers/035-feature-user-role-and-password-wiring.md`)+ spec(16/16 quality checklist 全通過、0 NEEDS CLARIFICATION)+ clarify(掃描無歧義、未改 spec)+ plan(Constitution Check 5/5 PASS — 首跑於 Principle IV 卡 gate、經 `/speckit-constitution` amend Constitution 至 v1.2.0〔W-WEBUI 軌道列舉擴 W-FW1–W-FW7 + 准動範圍納入「§4 授權下最小 UI 新增」〕後通過;Phase 0 research R-Q1~R-Q3 resolved)。整併 W-FW1-N1(user→roles 指派)+ W-FW1-N2(password UX):Part A 純後端補 `SystemManageUserOutput.user_roles` 真實填充 + transform DTO 收 `userRoles` delta 寫 `sys_user_role`(不需同步 Casbin g — enforcement 用 JWT role);Part B 修 `update_user` 密碼未 hash bug + drawer 加選填 password 欄 + 新增 `/auth/changePassword` 自助改密碼端點 + user-center 補修改密碼面板。比照 W-FW1~W-FW4。
- **Previous features**: W-F1 merge `430ada9` / W-F2 merge `ac79ed0` / W-F3 merge `04671d0` / W-F4 merge `ab658d7` / W-F5 merge `dff14c2` / W-F7 merge `62b3475` / W-F6 merge `5e38030` / F6 merge `a431215` / W-FA1 merge `b095d55` / W-FA2 merge `c5b7840` / W-FA3 merge `f23f38e` / F10 merge `8f0e84c` / F10.1 merge `48b70e6` / F10.2 merge `851ec79` / F11 merge `81ecb0d` / F9 merge `b2f910c` / F7 merge `136b1eb` / F7.1 merge `efe910e` / F7.2 merge `476ca88` / F8 merge `c2b0912` / W-F11 merge `d2d4c4c` / F12 merge `86e56e5` / F13 merge `4e9cf07` / F14 merge `1f20a0d` / 030 merge `0ed2e85` / 031 merge `a09d316` / 032 merge `8ccc4b4` / 033 merge `729dbf9` / 034 merge `ff5ea63`(均已 push、acceptance PASS;F13 為 DESIGN-A §6.1 Phase 5 P5 第一個 feature — rust 補實作 POST /auth/refreshToken refresh token 輪替;F14 為 P5 第二個也是最後一個 feature — nestjs 退場、DESIGN-B(rust-only)形態正式生效、DESIGN-A §6.1 全 14 feature 收尾;031 為 W-WEBUI 軌道第一個 feature — base-web user CRUD 接線、Constitution v1.1.0「W-WEBUI 受管例外」首次行使、11/11 C-V acceptance PASS;032 為 W-WEBUI 軌道第二個 feature — base-web menu CRUD 接線、Constitution v1.1.0「W-WEBUI 受管例外」第二次行使、11/11 C-V acceptance PASS、含 sys_menu id sequence desync + parentId 字串/數字落差 2 個 bug fix;033 為 W-WEBUI 軌道第三個 feature — base-web role CRUD 接線、Constitution v1.1.0「W-WEBUI 受管例外」第三次行使、14/14 C-V acceptance PASS、含 update_role status-drop 修正 + roleCode transform-layer code-lock 2 個 role-edit 正確性處理;034 為 W-WEBUI 軌道第四個也是最後一個 feature — base-web 角色菜單授權接線、Constitution v1.1.0「W-WEBUI 受管例外」第四次行使、13/13 C-V acceptance PASS、含 assign_routes 空清單 clear-all + 成功訊息 optional chaining 2 個 code review 修正)
<!-- SPECKIT END -->
