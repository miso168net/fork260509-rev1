...
<!-- SPECKIT START -->
- **Active feature**: —(無進行中 feature;031 user-crud-wiring 落地完成、多段式 commit + push + merge `a09d316` done)
- **Phase**: Done — 031 `031-user-crud-wiring`（W-FW1,W-WEBUI 軌道第一個 feature）落地(outer feature `10edf43` + merge `a09d316`、rust-api `2a24e9d`、base-web `1793b361`;base-web manage/user 新增/編輯/刪除/批次刪除 4 個 stub 操作接上 rust-api;rust-api systemManage alias 轉換層(SystemManageAddUserInput/SystemManageUpdateUserInput DTO + add/update transform handler + UpdateUserInput un-flatten 使 password 改 Option)+ base-web 接線(4 個寫入 service function + drawer handleSubmit + list delete handler);無 migration;acceptance C-V1~C-V11 11/11 PASS(含 CDP smoke 6 步驟、零 console error);subagent-driven-development + 兩段式 spec/code review + 整體 final review 執行;Constitution v1.1.0「W-WEBUI 受管例外」首次行使)。後續候選:W-FW1-N1 userRoles 接線、W-FW1-N2 password UX、observability W-F12/W-F13/W-F14。
