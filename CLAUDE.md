...
<!-- SPECKIT START -->
- **Active feature**: —(無進行中 feature;**DESIGN-A §6.1 全 14 個 application feature(F1–F14)收尾完成**、DESIGN-B(rust-only)形態正式生效)
- **Phase**: Done — F14 `029-design-a-to-b-cutover` 落地(outer `33758f0` + merge `1f20a0d`、rust-api `729d3c6`;acceptance C-V1~C-V11 11/11 PASS、C-V12 deferred;subagent-driven-development + 兩段式 spec/code review 執行;time gate 經 user 明示覆寫後 implement)。後續為非 DESIGN-A §6.1 範疇:observability(W-F12/W-F13/W-F14)、W-F6b acme 真實 cert。
