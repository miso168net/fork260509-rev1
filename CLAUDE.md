...
<!-- SPECKIT START -->
- **Active feature**: `038` `button-auth-completion`（W-FW8、W-WEBUI follow-up 軌道收尾 feature）—— spec-kit 設計鏈進行中：specify / clarify / plan 完成，plan `specs/038-button-auth-completion/plan.md`；待 `/speckit-tasks`。
- **Phase**: 設計中 — brainstorm(`docs/superpowers/038-feature-button-auth-completion.md`、4 Q 拍板) + spec(16/16 quality checklist 全通過、0 NEEDS CLARIFICATION) + clarify(掃描無歧義、未改 spec) + plan(Constitution Check 5/5 PASS、v1.3.0 amendment 已落地擴 W-FW1–W-FW8、Phase 0 research R-Q1~R-Q3 resolved — R-Q1 base-web 用 inline type alias EndpointTreeNode + primitive generic 不動 typings;R-Q2 既有 tree_endpoint by-controller 不符 spec by-resource 需求、transform handler 內 inline reshape 從 find_active().all() + BTreeMap group;R-Q3 既有資料 (path,method) unique 0 dup、reverse-map 用 in-memory HashMap defensive query)。整併 W-FW4-N1(button-auth modal 接通 Path A 複用 sys_endpoint + assign_permission、tree by resource、leaf label summary+method、v-permission 推遲)+W-FW6 Q4 順手項(assign_permission audit 補寫、trait 末尾 actor: &Actor、payload camelCase {roleId,domain,endpointIds:[...]}、entity_type='sys_role'、同 W-FW6 N3 體例)。base-web ≤ 2 檔(button-auth-modal.vue + system-manage.ts) + rust-api ~12 處 + 1 data migration(6 row Casbin seed)、0 schema migration、0 新 table、0 nestjs。比照 W-FW5/W-FW7/W-FW6。
- **Previous features**: W-F1 merge `430ada9` / W-F2 merge `ac79ed0` / W-F3 merge `04671d0` / W-F4 merge `ab658d7` / W-F5 merge `dff14c2` / W-F7 merge `62b3475` / W-F6 merge `5e38030` / F6 merge `a431215` / W-FA1 merge `b095d55` / W-FA2 merge `c5b7840` / W-FA3 merge `f23f38e` / F10 merge `8f0e84c` / F10.1 merge `48b70e6` / F10.2 merge `851ec79` / F11 merge `81ecb0d` / F9 merge `b2f910c` / F7 merge `136b1eb` / F7.1 merge `efe910e` / F7.2 merge `476ca88` / F8 merge `c2b0912` / W-F11 merge `d2d4c4c` / F12 merge `86e56e5` / F13 merge `4e9cf07` / F14 merge `1f20a0d` / 030 merge `0ed2e85` / 031 merge `a09d316` / 032 merge `8ccc4b4` / 033 merge `729dbf9` / 034 merge `ff5ea63` / 035 merge `7020899` / 036 merge `dd7de46` / 037 merge `10b5bee`(均已 push、acceptance PASS;F13 為 DESIGN-A §6.1 Phase 5 P5 第一個 feature — rust 補實作 POST /auth/refreshToken refresh token 輪替;F14 為 P5 第二個也是最後一個 feature — nestjs 退場、DESIGN-B(rust-only)形態正式生效、DESIGN-A §6.1 全 14 feature 收尾;031 為 W-WEBUI 軌道第一個 feature — base-web user CRUD 接線、Constitution v1.1.0「W-WEBUI 受管例外」首次行使、11/11 C-V acceptance PASS;032 為 W-WEBUI 軌道第二個 feature — base-web menu CRUD 接線、Constitution v1.1.0「W-WEBUI 受管例外」第二次行使、11/11 C-V acceptance PASS、含 sys_menu id sequence desync + parentId 字串/數字落差 2 個 bug fix;033 為 W-WEBUI 軌道第三個 feature — base-web role CRUD 接線、Constitution v1.1.0「W-WEBUI 受管例外」第三次行使、14/14 C-V acceptance PASS、含 update_role status-drop 修正 + roleCode transform-layer code-lock 2 個 role-edit 正確性處理;034 為 W-WEBUI 軌道第四個也是最後一個 feature — base-web 角色菜單授權接線、Constitution v1.1.0「W-WEBUI 受管例外」第四次行使、13/13 C-V acceptance PASS、含 assign_routes 空清單 clear-all + 成功訊息 optional chaining 2 個 code review 修正;035 為 W-WEBUI follow-up 軌道第一個 feature — 補完 W-FW1 留下的 user→roles 指派 + 密碼 UX、Constitution v1.2.0「W-WEBUI 受管例外擴 W-FW1–W-FW7 + §4 最小 UI 新增」首次行使、20/20 C-V acceptance PASS、含 change_password audit actor 型別 build-gate 修正 + user 抽屜角色欄 mock scaffolding 殘留去重 2 個收尾修正;036 為 W-WEBUI follow-up 軌道第二個 feature — 補完 W-FW2 留下的 menu 欄位持久化缺口 W-FW2-N1、sys_menu 加 query/buttons (JSONB) + fixed_index_in_tab (INTEGER) 3 欄、entity 同步、雙向 DTO 接通寫入+admin CRUD 讀回+runtime 路由 meta、buttons 不進 RouteMeta(Q2 拍板)、Constitution v1.2.0「W-WEBUI 受管例外」第二次行使、20/20 C-V acceptance PASS、base-web 0 改動 R-Q1/R-Q2 已驗證 modal UI 與 null 防護齊備、既有 audit 路徑自動涵蓋無需新邏輯;037 為 W-WEBUI follow-up 軌道第三個 feature — W-FW6 整併 W-FW4-N2 role home 持久化(sys_role 加 home_route_name VARCHAR NULL + 2 systemManage 端點 + base-web menu-auth-modal home 接真 API)+W-FW4-N3 assign_routes/users audit gap(audit_log::write_in_txn 補寫 whole-snapshot、camelCase keys)+W-FW3-N1 role code 安全改名(rust update_role detect code 變動同步 casbin_rule + notify_casbin_changed + 拿掉 W-FW3 code-lock)、Constitution v1.2.0「W-WEBUI 受管例外」第三次行使、C-V1~C-V27 全 27 條 PASS(含 CDP browser smoke via Edge :9229 驗 C-V23 home dropdown + C-V24 roleCode rename)、含 audit payload JSON keys camelCase 對齊 1 個 review fix)
<!-- SPECKIT END -->
