...
<!-- SPECKIT START -->
- **Active feature**: —(無進行中 feature;033 role-crud-wiring(W-FW3、W-WEBUI 軌道第三個 feature)落地完成、多段式 commit + push + merge `729dbf9` done)
- **Phase**: Done — 033 `033-role-crud-wiring` 落地(outer merge `729dbf9`、SHA pin `fcf7280`、rust-api `536bf88`、base-web `ceafe62a`;W-FW3、W-WEBUI 軌道第三個 feature、base-web role CRUD 接線;rust E1–E5 4 個 alias DTO + 4 個 transform handler + 4 條 route + E4 update_role status-drop 修正 + E5 Casbin seed migration;acceptance C-V1~C-V14 14/14 PASS;含 2 個 role-edit 正確性處理:① E4 update_role 補 `status: Set(...)` — status 真持久化(FR-006)② roleCode transform-layer code-lock — update transform 用既有 role 的 `code`/`pid`、防 Casbin policy 孤兒化(FR-007);subagent-driven-development + 兩段式 spec/code review;含 1 個編譯 bug fix:`RoleInput` 漏 re-export(`536bf88`))。後續候選:W-FW4 role-authorization-wiring、W-FW3-N1 role code 改名能力、W-FW1-N1 userRoles 接線、W-FW1-N2 password UX、observability W-F12/W-F13/W-F14。
- **Previous features**: W-F1 merge `430ada9` / W-F2 merge `ac79ed0` / W-F3 merge `04671d0` / W-F4 merge `ab658d7` / W-F5 merge `dff14c2` / W-F7 merge `62b3475` / W-F6 merge `5e38030` / F6 merge `a431215` / W-FA1 merge `b095d55` / W-FA2 merge `c5b7840` / W-FA3 merge `f23f38e` / F10 merge `8f0e84c` / F10.1 merge `48b70e6` / F10.2 merge `851ec79` / F11 merge `81ecb0d` / F9 merge `b2f910c` / F7 merge `136b1eb` / F7.1 merge `efe910e` / F7.2 merge `476ca88` / F8 merge `c2b0912` / W-F11 merge `d2d4c4c` / F12 merge `86e56e5` / F13 merge `4e9cf07` / F14 merge `1f20a0d` / 030 merge `0ed2e85` / 031 merge `a09d316` / 032 merge `8ccc4b4` / 033 merge `729dbf9`(均已 push、acceptance PASS;F13 為 DESIGN-A §6.1 Phase 5 P5 第一個 feature — rust 補實作 POST /auth/refreshToken refresh token 輪替;F14 為 P5 第二個也是最後一個 feature — nestjs 退場、DESIGN-B(rust-only)形態正式生效、DESIGN-A §6.1 全 14 feature 收尾;031 為 W-WEBUI 軌道第一個 feature — base-web user CRUD 接線、Constitution v1.1.0「W-WEBUI 受管例外」首次行使、11/11 C-V acceptance PASS;032 為 W-WEBUI 軌道第二個 feature — base-web menu CRUD 接線、Constitution v1.1.0「W-WEBUI 受管例外」第二次行使、11/11 C-V acceptance PASS、含 sys_menu id sequence desync + parentId 字串/數字落差 2 個 bug fix;033 為 W-WEBUI 軌道第三個 feature — base-web role CRUD 接線、Constitution v1.1.0「W-WEBUI 受管例外」第三次行使、14/14 C-V acceptance PASS、含 update_role status-drop 修正 + roleCode transform-layer code-lock 2 個 role-edit 正確性處理)
<!-- SPECKIT END -->
