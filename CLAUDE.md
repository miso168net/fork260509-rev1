...
<!-- SPECKIT START -->
- **Active feature**: —(無進行中 feature;030 systemManage status/gender 對齊收尾完成、DESIGN-B 形態下一般 follow-up enhancement)
- **Phase**: Done — 030 `030-systemmanage-status-gender-alignment` 落地(outer `681dcbe` + merge `0ed2e85`、rust-api `0e1fb95`;F14 後 CDP 全功能巡檢 follow-up — base-web↔rust-api enum 契約落差收尾;US1 `map_status` 消除 vue-i18n INVALID_ARGUMENT + US2 `Gender` PG enum/`sys_user.gender` 欄位/getUserList 篩選 + US3 create/update 寫入;acceptance C-V1~C-V13 13/13 PASS;subagent-driven-development + 兩段式 spec/code review + TDD 執行)。後續候選:base-web user create/edit CRUD 端到端接線 follow-up、observability W-F12/W-F13/W-F14。
