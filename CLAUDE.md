...
- **Phase**: Done — 032 `032-menu-crud-wiring` 落地(outer `d96aafa` + merge `8ccc4b4`、rust-api `149dc52`、base-web `b43634c0`;W-FW2 W-WEBUI 軌道第二個 feature、base-web menu CRUD 接線;acceptance C-V1~C-V11 11/11 PASS;含 2 個 acceptance 階段 bug fix:① sys_menu id sequence desync — `m20260522_c_sys_menu_id_seq_fix` setval migration(spec FR-016 受核准例外、user 明示核准)② parentId 字串/數字契約落差 — `deserialize_with` helper 雙格式相容;subagent-driven-development + 兩段式 spec/code review + TDD 執行)。後續候選:W-FW2-N1 menu query/buttons/fixedIndexInTab 持久化、W-FW1-N1 userRoles 接線、W-FW1-N2 password UX、observability W-F12/W-F13/W-F14。
