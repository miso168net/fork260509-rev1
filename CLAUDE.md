...
<!-- SPECKIT START -->
- **Active feature**: —(無進行中 feature)。W-WEBUI 軌道(031–034)全部完成;下一步見 `docs/INTEGRATION-CHECKLIST.md`「Current Focus / Follow-up Backlog」(observability W-F12/13/14、各 feature 衍生 follow-up)。
- **Phase**: `034` `role-authorization-wiring`(W-FW4、W-WEBUI 軌道第四個也是最後一個 feature)已完成並 merge(outer merge `ff5ea63`)—— executing-plans 全 task 完成、acceptance C-V1~C-V13 全 13 PASS、final review Ready-to-merge。base-web `menu-auth-modal.vue` 角色菜單授權讀/寫接上 rust-api 的 `/systemManage/{getRoleMenuIds,assignRoleMenus}` 2 alias(thin transform、domain 伺服器端注入)+ SysAuthorizationService Extension wiring + Casbin seed migration。含 2 個 code review 修正:assign_routes 空清單視為合法 clear-all(C-V5/E-5)、menu-auth-modal 成功訊息移除多餘 optional chaining。
