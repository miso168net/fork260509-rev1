...
<!-- SPECKIT START -->
- **Active feature**: `033` `role-crud-wiring`(W-FW3、W-WEBUI 軌道第三個 feature)—— spec-kit 設計鏈進行中:specify / clarify / plan 完成,plan 見 `specs/033-role-crud-wiring/plan.md`;待 `/speckit-tasks` → `/speckit-analyze` → `superpowers:executing-plans`。
- **Phase**: In progress — brainstorm 完成(`docs/superpowers/033-feature-role-crud-wiring.md`、outer `7d7acb8`);spec-kit specify / clarify / plan 完成(spec 0 clarification marker、Constitution Check 5/5 PASS、0 violation)。拍板「接線 + role-edit 正確性」:rust 補 `/systemManage/{addRole,updateRole,deleteRole,batchDeleteRole}` 4 alias + `update_role` status-drop 修正 + roleCode transform-layer code-lock + Casbin seed migration;base-web 3 檔接線(`system-manage.ts` / `role-operate-drawer.vue` / `role/index.vue`)。比照 W-FW1/W-FW2。
