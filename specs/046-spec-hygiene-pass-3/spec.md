...
2. **Given** 4 test file；**When** grep `wait_for_audit_row`；**Then** ≥12 callsite hits（每 ignored test ≥1 helper call）。
...
- **SC-005**：`rust-api/server/model/tests/common/audit_pipeline.rs` 新檔存在含 `pub async fn wait_for_audit_row` signature；4 test file 內 ≥12 個 callsite 使用 helper；`#[ignore = "requires dev stack drainer running…"]` 註解 ≥12 hit。
