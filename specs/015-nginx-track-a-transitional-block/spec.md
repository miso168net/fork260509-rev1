...
- [`docs/INTEGRATION-DESIGN-A-RUST-NESTJS.md`](../../docs/INTEGRATION-DESIGN-A-RUST-NESTJS.md) §2.2(nginx 路由分流規則 + TRANSITIONAL marker convention)、§3.2(nestjs-bound endpoint 列表 = refreshToken-only、嚴格不擴張)
...
- 既有 [`deploy/front-nginx/conf.d/default.conf.prod`](../../deploy/front-nginx/conf.d/default.conf.prod)(W-F6 落地、80 redirect 443 + 443 ssl、W-FA2 在 443 server 加 inline block;prod 80 只 redirect、不加 TRANSITIONAL block)
...
- 既有 W-FA1 落定的 nestjs container port = 9528 + HTTP global prefix `/v1`(per `fork260509-soybean-admin-nestjs/backend/apps/base-system/src/main.ts:88` `setGlobalPrefix('v1')` + `EXPOSE 9528`)
- 既有 [`base-web/src/service/api/auth.ts:30`](../../base-web/src/service/api/auth.ts)(SPA `fetchRefreshToken` 已 call `POST /auth/refreshToken` body `{refreshToken: <token>}`、透 VITE_SERVICE_BASE_URL=/api 變 `/api/auth/refreshToken`、W-FA2 補上 nginx routing 即可)
- 既有 [`fork260509-soybean-admin-nestjs/backend/apps/base-system/src/api/iam/rest/authentication.controller.ts:58-59`](../../fork260509-soybean-admin-nestjs/backend/apps/base-system/src/api/iam/rest/authentication.controller.ts)(`@Controller('auth')` + global prefix `/v1` → endpoint `POST /v1/auth/refreshToken`、`@Public()` 不需 JWT、`@Body('refreshToken')` 對齊 SPA body)

**Scope summary**:rev1 deploy 階段 Track DESIGN-A 三件套第二個 feature。**把 `POST /api/auth/refreshToken` 路由補上 nginx 反代到 nestjs upstream**、用 inline TRANSITIONAL marker block 包在 default.conf / default.conf.prod 內;走 **variable proxy_pass + resolver lazy DNS** 設計讓 nginx 在 default profile / DESIGN-B 退場時不因 nestjs 缺席而 fail。範疇刻意收緊到「nginx 加 1 條 exact-match location + variable proxy_pass 機制 + 3 處 marker block 結構」,**不含** nestjs application 改動(F10)、不含 rust 補實作(F13)、不含實際 cutover(F14)、不含 rate limiting。

**Commit 模式**:**單段 commit**(per W-F1~W-F7 + W-FA1 慣例)— 只動 outer repo、不動 worktree、不動 nestjs fork。

**範疇外**:nestjs application source code 改動 / rust-api 補實作 refreshToken / DESIGN-A → DESIGN-B 實際 cutover 執行 / rate limiting / refreshToken 防爆機制 / upstream keepalive 補回 / nestjs 任何非 refreshToken endpoint route / nginx access log JSON 格式對齊 / nestjs image push 到 container registry。
...
- **Q2**: nginx TRANSITIONAL block 怎麼做 profile-aware?(關鍵 constraint:default profile / DESIGN-B 退場時 nginx 不能因沒 nestjs container 就啟動 fail) → **A: variable proxy_pass + inline TRANSITIONAL block**。default.conf 內 inline marker block;用 `resolver 127.0.0.11` + `set $nestjs_upstream "nestjs:9528"` + `proxy_pass http://$nestjs_upstream/v1/auth/refreshToken`(變數式 proxy_pass、DNS 延遲到 request 時)。Pros:不加新 compose override file、operator UX 不變;default profile nginx 不 fail(只是 request 送到 refreshToken 會回 502、不影響其他 route);DESIGN-B 退場 = 單一 block 刪除(符 marker convention)。Cons:失 upstream keepalive(refreshToken 低頻、影響小);需加 resolver directive。對比方案 A(`track-a.inc` + 新 `docker-compose.track-a.yml` override)需 operator 加 `-f` flag;方案 C(upstream block + 手動退場)破壞 default profile 可用性 invariant。

- **Q3**: TRANSITIONAL block 進哪些 nginx config 檔? → **A: dev + prod 都加**。`default.conf`(dev/80+443)與 `default.conf.prod`(prod/443 ssl;prod 80 只 redirect、不加)都加 inline TRANSITIONAL block。對齊 W-FA1 FR-012 明示「prod 不暴露 nestjs host port — 僅內部訪問、透 W-FA2 nginx routing」;dev/prod 3 個 server block 都加 location 條目、設計一致、prod 也能走 refreshToken。
...
- **自然推論**:**marker block 在每個 server 各自獨立 inline** — 不抽 snippet 到 conf.d/track-a.inc(避免 Q2 拍板 inline 的隱含取捨被反悔);3 處 inline 雖有 duplication 但每個 server block self-contained、F14 cutover 用 `sed '/>>>>> TRANSITIONAL BEGIN/,/<<<<< TRANSITIONAL END/d'` 機械刪除即可。
...
1. **Given** dev stack `--profile track-a up` 7 service healthy、W-FA2 落地的 nginx config 已 reload,**When** `docker compose exec front-nginx nginx -t`,**Then** exit 0 syntax OK
2. **Given** stack 已啟,**When** `curl -X POST -H "Content-Type: application/json" -d '{"refreshToken":"invalid-test-token"}' http://127.0.0.1:11080/api/auth/refreshToken`,**Then** response 不是 HTTP 404(證明不被 rust-api `/api/` prefix 攔)、是 nestjs ApiRes envelope(具體 code/msg 視 nestjs token 驗證邏輯、W-FA2 不負責 business 正確性、留 F10)
...
1. **Given** stack 已啟(track-a),**When** `docker compose -f docker-compose.yml -f docker-compose.dev.yml --profile track-a down --remove-orphans` + `docker compose -f docker-compose.yml -f docker-compose.dev.yml up -d --wait`(不帶 profile),**Then** 6 service healthy(無 nestjs)、`docker compose exec front-nginx nginx -t` OK
2. **Given** default profile stack 已啟,**When** `curl -o /dev/null -w "%{http_code}\n" -X POST -H "Content-Type: application/json" -d '{"refreshToken":"x"}' http://127.0.0.1:11080/api/auth/refreshToken`,**Then** HTTP 502 / 504(nginx 報 upstream 不可達、不是 crash 不是 404)
...
2. **Given** dev cert 已 seed 進 named volume(per W-F6 quickstart),**When** `docker compose -f docker-compose.yml -f docker-compose.prod.yml --profile track-a up -d --wait`,**Then** 7 service healthy + `docker compose exec front-nginx nginx -t` OK
3. **Given** prod baseline + track-a 啟動(seed cert),**When** `curl -k -X POST -H "Content-Type: application/json" -d '{"refreshToken":"x"}' https://127.0.0.1:11443/api/auth/refreshToken`,**Then** 行為對齊 dev US1.2(nestjs envelope、非 404、非 502 — 因 nestjs 在 track-a 啟用);若 prod cert 未 seed,降為 syntax-only verify(C-V11 W-F6 既有降級慣例)
...
3. **Given** W-FA2 落地的 default.conf / default.conf.prod,**When** `sed '/>>>>> TRANSITIONAL BEGIN/,/<<<<< TRANSITIONAL END/d' <file>` 對 2 個 nginx config 都跑、產出 diff 文件臨時 mount 進 nginx 跑 `nginx -t`,**Then** 刪除後 `nginx -t` 仍 exit 0(證 F14 cutover 機械刪除路徑可行、不會 break)
...
| E-8 | nginx -t 時 nestjs container 不在(default profile)| variable proxy_pass + resolver lazy DNS 設計確保 nginx -t 仍通(不在 startup 時 resolve hostname);per Q2 拍板 + FR-017 |
...
- **FR-003**: TRANSITIONAL block MUST 用 marker comment `# >>>>> TRANSITIONAL BEGIN — DESIGN-A → DESIGN-B 拔除點(per W-FA2) <<<<<` 開始、`# <<<<< TRANSITIONAL END >>>>>` 結束(對齊 DESIGN-A §2.2 marker convention)。
- **FR-004**: TRANSITIONAL location block MUST 用 `location = /api/auth/refreshToken`(`=` exact match、優先順序高於 `/api/` prefix → 不被既有 rust-api routing 攔截)。
- **FR-005**: TRANSITIONAL location block MUST 含 `resolver 127.0.0.11 valid=10s ipv6=off;`(Docker 內建 DNS、lazy 解析機制、nestjs container 重啟後自動 re-resolve)。
- **FR-006**: TRANSITIONAL location block MUST 用 `set $nestjs_upstream "nestjs:9528";` 設變數(觸發 variable proxy_pass、避 startup-time DNS lookup;per Q2 拍板 lazy DNS 機制)。
- **FR-007**: TRANSITIONAL location block MUST 用 `proxy_pass http://$nestjs_upstream/v1/auth/refreshToken;`(顯式對齊 nestjs GLOBAL_PREFIX `/v1`、修正 DESIGN-W §4.3 草稿漏 `/v1` prefix + port 3000 → 9528 兩處錯誤)。
- **FR-008**: TRANSITIONAL location block MUST 用 `include /etc/nginx/snippets/proxy_headers.inc;` 沿用 W-F5/W-F6 共用 5 header(Host / X-Real-IP / X-Forwarded-For / X-Forwarded-Proto / X-Request-ID)+ 60s timeout。
- **FR-009**: W-FA2 MUST 為 **單段 commit**(對齊 W-F1~W-F7 + W-FA1 慣例、只動 outer repo)。
- **FR-010**: `CLAUDE.md` §5.2.1 MUST 加 track-a 模式下 curl `POST /api/auth/refreshToken` 範例(對齊 dev 啟動範例風格、放在第 4 個啟動模式範例「DESIGN-A 路線 dev」之後)。
- **FR-011**: `docs/INTEGRATION-CHECKLIST.md` MUST 更新:Phase W-7 roadmap 表 W-FA2 row 「進行中」→「完成」+ Current Focus 更新 + 已完成里程碑加 W-FA2 條目。
- **FR-012**: W-FA2 MUST 不引入任何 nginx upstream block(全程用 variable proxy_pass + resolver、per Q2 拍板;副作用是失 keepalive、對 refreshToken 低頻場景可接受)。
...
- **FR-014**: W-FA2 MUST 不引入新 docker compose override file(per Q2 拍板捨棄 Option A、保 operator UX 不變;啟動命令仍是 `docker compose -f -f --profile track-a up`、無新 `-f` flag)。
- **FR-015**: W-FA2 MUST 不引入新 nginx config file(per 自然推論 inline、不抽 snippet 到 `conf.d/track-a.inc`)。
- **FR-016**: W-FA2 acceptance MUST 含「`sed` 機械刪除 marker block 後 `nginx -t` 仍通」測試(US4.3、證 F14 cutover 路徑乾淨)。
- **FR-017**: W-FA2 落地後 default profile(不帶 `--profile track-a`)啟動 stack MUST 仍能正常啟動 6 service healthy + nginx -t OK(per FR-014 + Q2 lazy DNS);`POST /api/auth/refreshToken` 在 default profile 下 MUST 回 HTTP 502 / 504 而非 nginx crash 或 HTTP 404。

### Non-Functional Requirements

- **NFR-001**: nginx config 改動量 SHOULD ≤ 35 行(3 個 server block 各 ~9 行 TRANSITIONAL block + 5-10 行 doc)。
- **NFR-002**: variable proxy_pass + resolver 失 upstream keepalive、refreshToken response latency SHOULD ≤ 既有 rust-api login flow latency + 20ms 額外 DNS resolution overhead(可接受、refreshToken 低頻場景、~每小時 1 次/user)。
...
- **`deploy/front-nginx/conf.d/default.conf` TRANSITIONAL block × 2**(dev 80 server + dev 443 server)— ~9 行 nginx inline block、含 marker comment + location = /api/auth/refreshToken + resolver + set + proxy_pass + include
- **`deploy/front-nginx/conf.d/default.conf.prod` TRANSITIONAL block × 1**(prod 443 server)— 同 dev block 結構
- **`CLAUDE.md` §5.2.1**(改)— 加 track-a 模式下 curl POST refreshToken 範例
- **`docs/INTEGRATION-CHECKLIST.md`**(改)— W-FA2 row + Current Focus + 已完成里程碑
...
- **SC-001**: dev stack `--profile track-a up -d --wait` + `curl -X POST /api/auth/refreshToken` 透 host port 11080 → HTTP status code 不是 404(證 nginx route 不被 rust-api `/api/` 攔)、response body 是 nestjs ApiRes envelope shape(具 `code` / `msg` / `data` 欄位)。
- **SC-002**: dev stack default profile(不帶 profile)`up -d --wait` 6 service healthy、`docker compose exec front-nginx nginx -t` exit 0、F6 login flow(`POST /api/auth/login` + `GET /api/route/isRouteExist`)全 PASS。
- **SC-003**: dev stack default profile + `curl POST /api/auth/refreshToken` HTTP status code = 502 / 504(nginx upstream 不可達、不是 crash);front-nginx container 自身 `docker compose ps front-nginx` 仍 `(healthy)` 狀態。
- **SC-004**: prod compose `--profile track-a config --quiet` exit 0;prod baseline + track-a + seed cert 啟動後 7 service healthy + nginx -t OK。
- **SC-005**: `sed '/>>>>> TRANSITIONAL BEGIN/,/<<<<< TRANSITIONAL END/d'` 對 default.conf + default.conf.prod 刪除後產出的 diff 跑 `nginx -t`(臨時 volume mount)exit 0;證 F14 cutover 機械刪除路徑可行。
- **SC-006**: `git diff HEAD -- base-web/ rust-api/ fork260509-soybean-admin-nestjs/` 無輸出(per FR-013 三邊零改動)。
- **SC-007**: W-FA2 為單段 commit、`git log --oneline -1` 為 W-FA2 主要落地 commit、無「兩段式」紀律(對齊 W-FA1 模式)。

## Assumptions

- **A-001**: nestjs fork repo `apps/base-system/src/api/iam/rest/authentication.controller.ts:58-83` `@Public @Post('refreshToken')` endpoint 已實作(W-FA2 brainstorm 階段 2026-05-18 grep 確認、business 邏輯正確性留 F10)。
- **A-002**: nestjs `@Body('refreshToken')` 接 SPA fetchRefreshToken 送的 `{refreshToken: <token>}` body 格式 — 對齊 base-web `auth.ts:30` 既有 wiring(SPA 不改、W-FA2 only nginx routing)。
...
- **A-006**: refreshToken endpoint 低頻場景下、失 upstream keepalive 不會造成 noticeable latency 問題(business 場景:user 操作 token 過期才觸發、~每小時 1 次/user;對比 login flow 每次 login 一次)。
...
- **OOS-002**: rust-api 補實作 refreshToken — F13 範疇(W-FA2 是過渡 wire-up、refreshToken 仍 nestjs handle)。
- **OOS-003**: DESIGN-A → DESIGN-B 實際 cutover 執行 — F14 範疇(W-FA2 只 dry-run 驗 marker block 可機械刪)。
...
- **OOS-008**: nestjs image push 到 container registry — W-FA3 範疇。
