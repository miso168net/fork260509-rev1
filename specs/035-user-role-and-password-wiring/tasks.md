...
- [ ] T010 [US2] rust-api transform handler password wiring（`server/api/src/admin/sys_system_manage_api.rs`）：`add_user_for_systemmanage` —— `input.password` 為 `Some` 用該值、`None` 沿既有預設密碼（`123456`）；`update_user_for_systemmanage` —— `input.password` 原樣傳入 `UpdateUserInput.password`（`Some` 經 T008 修正後 hash、`None` 不動 password）（依 T008、T009；同組 handler 不與 T005 平行；data-model.md B2）
- [ ] T011 [P] [US2] base-web `src/views/manage/user/modules/user-operate-drawer.vue` 加**選填** password 欄：表單加 `NInput type="password"`、`model` 加 `password`、`handleSubmit` 送出帶 `password`（留空＝不送 / 視為未提供）；userRoles 欄 W-FW1 已接線、**不動**（§4 W-FW5 amendment 授權的最小 UI 新增；data-model.md B2 / B4）
...
- [ ] T018 [US3] rust-api Casbin seed 查證與處理：對照 `/auth/getUserInfo`（同 protected router）在 `casbin_rule` 的處理 —— 若 protected router 受 Casbin enforce 且 getUserInfo 有 all-role `p` policy → 新增 `/auth/changePassword` Casbin seed migration（全 role allow、`POST` + register `mod.rs`/`lib.rs`）；若不受 enforce → 無需 seed、記錄查證結論（依 T017；data-model.md B3d、research.md R-Q2）
...
- [ ] T020 [P] [US3] base-web 新增 `fetchChangePassword(data)` service function（`POST /auth/changePassword`，body `{ currentPassword, newPassword }`）in `src/service/api/system-manage.ts` 或既有 auth service 檔（依 changePassword 歸屬）（data-model.md「base-web service function」）
