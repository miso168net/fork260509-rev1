...
- [ ] T015 (optional、**0 拾取為 baseline / user 確認後才執行**) outer feature branch commit 3（implementer-stage expansion candidate (a) 拾取）：改 `specs/045-facade-atomicity-pass/contracts/verification-commands.md` C-V5 段加 inline comment「post-047 PASS」說明 045 C-V5 grep expectation 在 047 fix 後現可重新跑通；~1-3 line spec docs edit。per [research R-6.1](./research.md)、屬 implementer-stage expansion budget ≤3（0/3 拾取 = full baseline、不違 FR-011）。對應 FR-011。
