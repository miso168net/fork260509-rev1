...
**Independent Test**：dev stack 12 service healthy 跑著、from host 對 `http://127.0.0.1:11080/api/sandbox/simple-api-key` 發 4 個 curl：(a) valid `x-api-key: test-api-key` → HTTP 200 + body `code:0, success:true`；(b) invalid `x-api-key: bogus-XYZ` → **HTTP 401** + response header `WWW-Authenticate: ApiKey` + body `code:5004, success:false`；(c) 完全不送 `x-api-key` header → **HTTP 401** + 同 header + body `code:5003, success:false`；(d) 送 empty `x-api-key:` header → **HTTP 401** + 同 header + body `code:5003, success:false`。

**Acceptance Scenarios**：

1. **Given** dev stack 12 service healthy、`/sandbox/simple-api-key` 由 `protect_route` 註冊保護、validator set 已含 `test-api-key`；**When** `curl -H "x-api-key: test-api-key" http://127.0.0.1:11080/api/sandbox/simple-api-key`；**Then** HTTP status 為 `200` 且 body `code` 為 `0` 且 `success` 為 `true`（valid key 通過、handler 執行）。
...
### User Story 2 — Developer 跑 `cargo test` 看到 4 個 unit test 涵蓋 middleware 全行為（Priority: P1）🎯 MVP

維護 `api_key_middleware` 的 developer / AI implementer / code reviewer 跑 `cargo test -p server-core sign::api_key_middleware` 應看到 4 個 unit test 全 PASS，涵蓋：missing key → 401 + header / invalid key → 401 + header / valid key → 200 pass-through / non-protected path → 200 pass-through。Unit test 不需 dev stack drainer / `TEST_DATABASE_URL` / `#[ignore]`、跟著 `cargo test` 一般執行流程跑，提供快速回歸防護。

**Why this priority**：unit test 是回歸防護的最低成本層、跟 production code 同檔便於修改、無需 DB / dev stack 即可跑；本 US 達成 045-N1 原文「補真實的『無效 key → 401』測試（單元 or integration）」的單元測試承諾。MVP-worthy 因為「測試證據」是 verifiable / reproducible quality gate；沒有 unit test，US1 的行為更改容易被未來 refactor 破壞而無察。

**Independent Test**：在 rust-api worktree 跑 `cargo test -p server-core sign::api_key_middleware -- --nocapture`、輸出含 4 個 test `... ok`、`test result: ok. 4 passed; 0 failed`、不需任何外部 dependency（dev stack 不需跑）。

**Acceptance Scenarios**：

1. **Given** `api_key_middleware.rs::tests` 含 `missing_api_key_returns_401_with_www_authenticate`；**When** `cargo test` 跑該 test；**Then** test PASS、`response.status() == 401`、`headers.get("WWW-Authenticate")` 等於 `"ApiKey"`、body 解析後 `code == 5003 && success == false`。
2. **Given** `api_key_middleware.rs::tests` 含 `invalid_api_key_returns_401_with_www_authenticate`；**When** 跑該 test；**Then** test PASS、`response.status() == 401`、同 header、body `code == 5004 && success == false`。
3. **Given** `api_key_middleware.rs::tests` 含 `valid_api_key_passes_through`；**When** 跑該 test；**Then** test PASS、`response.status() == 200`、body 為 dummy handler 回的 OK。
4. **Given** `api_key_middleware.rs::tests` 含 `non_protected_path_passes_through`；**When** 跑該 test；**Then** test PASS、`response.status() == 200`、middleware short-circuit 不執行 validation、body 為 dummy handler 回的 OK。
...
- **FR-008**：`rust-api/server/core/src/sign/api_key_middleware.rs::tests` MUST 新增 4 個 unit test：(a) `missing_api_key_returns_401_with_www_authenticate`；(b) `invalid_api_key_returns_401_with_www_authenticate`；(c) `valid_api_key_passes_through`；(d) `non_protected_path_passes_through`。每 test 使用 `tower::ServiceExt::oneshot` 驅 minimal `axum::Router` + dummy handler、send mock `Request`、assert response status + headers + body envelope。
...
- **SC-002**：`cargo test -p server-core sign::api_key_middleware -- --nocapture` 跑 4 個 unit test、輸出 `test result: ok. 4 passed; 0 failed`、無 `#[ignore]` 標註、跑時間 < 1 second（unit test 無外部 dependency）。
- **SC-003**：dev stack 跑著、from host 對 `http://127.0.0.1:11080/api/sandbox/simple-api-key` 發 4 case curl：(a) valid key → HTTP 200 + body `code:0`；(b) invalid key → HTTP 401 + response header `WWW-Authenticate: ApiKey` + body `code:5004`；(c) 無 header → HTTP 401 + 同 header + body `code:5003`；(d) empty header → HTTP 401 + 同 header + body `code:5003`。
- **SC-004**：`rust-api/server/core/src/sign/api_key_middleware.rs` 內 `#[cfg(test)] mod tests` 包含 4 個 test function、命名對齊 FR-008 (a)(b)(c)(d) 規定；grep `pub async fn api_key_middleware` 在 production code 區仍 1 hit（function signature 不變、僅內部 error response 構造改）。
