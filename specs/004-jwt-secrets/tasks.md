...
- [ ] T003 修改 `rust-api/server/config/src/lib.rs` — 加 `pub mod secret_loader;` 一行（依字母順序）
- [ ] T004 [P] 修改 `rust-api/server/config/src/model/jwt_config.rs` — 擴 doc comment 說明 `_FILE` precedence + strict validation rules（per [`data-model.md`](./data-model.md) §E3）；struct 不動（per spec FR-014）
- [ ] T005 修改 `rust-api/server/config/src/config_init.rs` — 在 `init_from_file` 內、`global::init_config::<JwtConfig>(config.jwt).await` 之前、加入 2 行 integration：(a) `if let Some(v) = secret_loader::load_secret_from_file_if_set("APP_JWT_JWT_SECRET") { config.jwt.jwt_secret = v; }`（_FILE precedence override）(b) `secret_loader::validate_jwt_secret(&config.jwt.jwt_secret);`（strict validation panic on fail）；per [`data-model.md`](./data-model.md) §E2
...
- [ ] T006 [P] [US1] 修改 `rust-api/server/resources/application.yaml` — `jwt.jwt_secret` 值改為 `"change-me-jwt-secret"`（在 PLACEHOLDER_SECRETS blacklist 內、boot 必 panic + 友善 error msg）；保留既有 issuer / expire 不動；加 yaml comment 註明 `# F1.1: prod 必透過 APP_JWT_JWT_SECRET_FILE 或 APP_JWT_JWT_SECRET envvar 設真 secret；產生方式：openssl rand -hex 32`；per [`data-model.md`](./data-model.md) §E4
- [ ] T007 [P] [US1] 修改 `rust-api/server/resources/application-test.yaml` — `jwt.jwt_secret` 改為 32-char dev secret（例：`"dev-test-secret-padded-to-thirty-two!"`、38 chars > 32、合規通過 strict validation；確切字串 implementer 可選其他 ≥32-char 合規值）；保留既有 issuer / expire 不動；加 yaml comment 註明 `# F1.1: test secret、32-char 合規值通過 strict validation；公開於 yaml 不影響 prod`
...
- [ ] T011 [P] [US1] 新建 `rust-api/server/initialize/tests/jwt_boot_integration.rs` — 2-3 個 integration test fn（透過 `std::panic::catch_unwind` 攔 panic）：
...
- [ ] T012 [P] SC-001 ~ SC-004 unit test verify：`cd rust-api && cargo test -p server-config jwt_secret_validation 2>&1 | tail -10` — 預期所有 unit test 綠燈
- [ ] T013 [P] SC-005 ~ SC-007 integration test verify：`cd rust-api && cargo test --test jwt_boot_integration 2>&1 | tail -10` — 預期 boot integration test 全 pass
- [ ] T014 [P] SC-008 grep verify resources files：
  ...
  - `grep -E '(openssl rand|_FILE|APP_JWT)' rust-api/server/resources/application.yaml.example` 預期 ≥3 hits（含 openssl 指引 + `_FILE` envvar + bare envvar 範例）
- [ ] T015 SC-009 F3 既有 acceptance test 仍 pass 驗：`export TEST_DATABASE_URL=postgresql://...` + `cd rust-api && cargo test --test soft_delete_basics --test soft_delete_audit_integration --test soft_delete_auth_gate -- --ignored 2>&1 | tail -15` — 預期全綠（F1.1 不改 JWT 簽驗 path / Claims struct、F3 G9 FR-028 layer 不動）
- [ ] T016 [P] SC-010 + SC-011 verify：
  - `ls specs/004-jwt-secrets/contracts/claim-contract.md` 預期 file 存在
  - `grep -cE '^\| `(sub|exp|iss|aud|iat|nbf|jti|username|role|domain|org)`' specs/004-jwt-secrets/contracts/claim-contract.md` 預期 ≥11（11 fields 都列）
  - `cd rust-api && cargo check 2>&1 | tail -5` 預期 `Finished dev profile` 0 errors / 0 warnings
- [ ] T017 跑 [`quickstart.md`](./quickstart.md) Step 1-10 完整 verification（含 application.yaml placeholder 驗 / 無 env 觸發 panic / bare envvar / `_FILE` / `_FILE` precedence / missing file panic / `_FILE` 內 placeholder 仍 panic / unit test / F3 既有 acceptance test / claim contract doc + cargo check / integration boot test）— 所有步驟 Pass 才算 F1.1 達成
...
Before implementation (T001 執行時填入):
  application.yaml jwt_secret 既有值: __ (預期 "soybean-admin-rust")
  application-test.yaml jwt_secret 既有值: __ (預期同上)
  config_init.rs:66 global::init_config::<JwtConfig> callsite: __ (預期 exist)
  validate_jwt_secret in rust-api/server/ 命中數: __ (預期 0)
  load_secret_from_file_if_set in rust-api/server/ 命中數: __ (預期 0)
  application.yaml.example 存在?: __ (預期 No)

After implementation (T012 / T013 / T014 / T016 執行時填入):
  application.yaml jwt_secret: "change-me-jwt-secret" ✓ (SC-008 pass)
  application-test.yaml jwt_secret: 32-char dev secret ✓
  application.yaml.example 存在 + 含 envvar / _FILE 範例 ✓
  validate_jwt_secret 命中數 in server/config/: 1 (definition) + N (tests)
  load_secret_from_file_if_set 命中數: 1 (definition) + ≥1 (config_init call) + N (tests)
  cargo check 全 workspace: 0 errors / 0 warnings ✓ (SC-011 pass)
  Unit test (server-config jwt_secret_validation): 全綠 ✓ (SC-001 ~ SC-004)
  Integration test (jwt_boot_integration): 全綠 ✓ (SC-005 ~ SC-007)
  F3 既有 acceptance test: 全綠 ✓ (SC-009)
