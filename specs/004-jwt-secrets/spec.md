...
**Feature Branch**: `004-jwt-secrets`
**Created**: 2026-05-14
**Status**: Draft
**Input**: User description: "讀取 docs/superpowers/004-feature-jwt-secrets.md 的設計文件產出規格書"

**Source brainstorming**: [`docs/superpowers/004-feature-jwt-secrets.md`](../../docs/superpowers/004-feature-jwt-secrets.md)（2026-05-14 superpowers:brainstorming session 產出、5 個 clarification 拍板）

**Authoritative parents**:
- [`docs/INTEGRATION-DESIGN-A-RUST-NESTJS.md`](../../docs/INTEGRATION-DESIGN-A-RUST-NESTJS.md) §3.3（JWT 簽章共識）、§6.1 F1、§6.2 依賴序
...
- [`specs/001-response-shape-alignment/spec.md`](../001-response-shape-alignment/spec.md)（F4，已完成）
- [`specs/002-soft-delete-infrastructure/spec.md`](../002-soft-delete-infrastructure/spec.md)（F3，已完成；F3 G9 加 jwt_auth_middleware FR-028 軟刪 user 8888 check、F1.1 不破壞此既有 layer）
- [`specs/003-audit-log-infrastructure/spec.md`](../003-audit-log-infrastructure/spec.md)（F2.1，已完成）
...
### Session 2026-05-14（brainstorming 階段拍板、5 項）
...
- Q: claim 欄位 standardize 範圍？既有 11 個（sub/exp/iss/aud/iat/nbf/jti 標準 + username/role/domain/org custom）。 → A: **保留現狀 + 加 doc reference**。Claims struct 不動於 F1.1、只新增 contract spec doc 明示 11 fields 用途。F10 refresh-token-bridge 階段補 `token_type` claim 時可正常擴（不動 F1.1 既有、backward compat）。
...
F1.1 的 4 個交付片段（_FILE loader / strict validation / yaml placeholder + example template / claim contract doc）**並非獨立可交付**：
...
13. **Given** F1.1 完成、查 `specs/004-jwt-secrets/contracts/claim-contract.md`，**Then** doc 列 Claims 11 fields（sub/exp/iss/aud/iat/nbf/jti 標準 + username/role/domain/org custom）+ 用途說明 + F1.2 / F10 預留 future extensions（`token_type`、`kid`、`family_id`） 註記
...
> **註**（per analyze 2026-05-14 M1 errata）：既有 `config_init.rs:522/590` `assert!` 實際在 `#[tokio::test]` test fn 內（非 production validation path）。F1.1 production strict validation 為**新增**（不是取代既有 production check — production 既有無 validation）；test 內 `assert!` 可保留（驗 config loader populates field non-empty、與 production validate 並行）或選擇升級為 `validate_jwt_secret` 呼叫（測試行為一致性）。Production validation 透過 tasks.md T005 在 `init_from_file` 內 `init_config::<JwtConfig>` 之前 inserted。FR-004 wording 偏向「希望最終所有 jwt_secret 檢查點都走 validate_jwt_secret」、實作上 production 是 add、test 可選 replace。
...
- **FR-013**：F1.1 MUST 新建 `specs/004-jwt-secrets/contracts/claim-contract.md`：列 `server-core::web::auth::Claims` 既有 11 fields（sub / exp / iss / aud / iat / nbf / jti / username / role / domain / org）+ 用途 + F1.2 / F10 預留 future extensions（`token_type` / `kid` / `family_id`） 註記
