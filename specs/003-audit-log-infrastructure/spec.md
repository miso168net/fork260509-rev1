...
**Feature Branch**: `003-audit-log-infrastructure`
**Created**: 2026-05-14
**Status**: Draft
**Input**: User description: "讀取 docs/superpowers/003-feature-audit-log-infrastructure.md 的設計文件產出規格書"

**Source brainstorming**: [`docs/superpowers/003-feature-audit-log-infrastructure.md`](../../docs/superpowers/003-feature-audit-log-infrastructure.md)（2026-05-14 superpowers:brainstorming session 產出、7 個 clarification 拍板）

**Authoritative parents**:
- [`docs/INTEGRATION-DESIGN-A-RUST-NESTJS.md`](../../docs/INTEGRATION-DESIGN-A-RUST-NESTJS.md) §1.5（全域 audit 紀律）、§5.2.1（audit 完整性）、§6.1 F2、§6.2 依賴序
...
- [`specs/001-response-shape-alignment/spec.md`](../001-response-shape-alignment/spec.md)（F4，已完成；envelope code 沿用 F4 namespace）
- [`specs/002-soft-delete-infrastructure/spec.md`](../002-soft-delete-infrastructure/spec.md)（F3，已完成；F2.1 收斂 F3 既有 `audit_log::write_in_txn` 為單一 AuditEvent 入口、refactor F3 既有 callsite）

**Scope summary**：rust admin 全 write 操作（INSERT / UPDATE / SOFT_DELETE / RESTORE）走**單一 audit context API**、業務 + audit 必同 transaction、`sys_operation_log` schema 擴 4 個結構化欄位（`operation` enum + `entity_id` + `payload_before` JSONB + `payload_after` JSONB）、敏感欄位透過 `AuditSerialize` trait 自動 redact、HTTP middleware audit 整合同一 context、新增 CI lint 守 service 內 INSERT/UPDATE 必有 audit 呼叫。**F2.1 範圍刻意縮限**：outbox 模式 + Redis subscriber TTL fallback + HARD_DELETE audit 全留 F2.2 / F12 階段獨立交付。

## Clarifications

### Session 2026-05-14 (clarify phase)

- Q: HTTP middleware + service-level audit 雙寫 row 怎麼鎖？ → A: **Always double-write**。同一 admin HTTP write 在 sys_operation_log 留 2 row — 1 個 HTTP 視角（method=POST/PUT/PATCH/DELETE 等）+ 1 個 service-level（method=INTERNAL）。middleware 不做 dedupe 檢測。Forensic 兩個視角都有獨立價值、實作最簡。
- Q: HTTP middleware audit row 的 entity_type 怎麼填？ → A: **Hybrid rule**。URL match `/api/sys-(user|role|menu|domain|organization|endpoint|access-key)/*` 7 條 admin pattern → entity_type 對應填 `sys_<x>`（kebab → snake）；不 match → fallback sentinel `"http_event"`。理由：admin URL pattern 穩定、reader 可 grep `entity_type='sys_user'` 抓到跨 HTTP + service-level 雙視角的 audit row；非 admin endpoint（如 `/api/auth/login`）不污染 entity_type namespace。
- Q: 既有 migration `datas/*` seeding INSERT（如 insert_sys_user / insert_sys_role 等 5 個）是否在 F2.1 audit 範圍？ → A: **Exempt**。F2.1 audit 範圍只含 application runtime write（service handler + HTTP middleware）；migration `datas/*` INSERT **不**寫 audit；seeding 紀錄走 git tracked migration 檔案本身。理由：migration 是 deploy/bootstrap-time、無 application-level Actor、audit 核心目的是 admin runtime 操作可追溯。

### Session 2026-05-14（brainstorming 階段拍板、7 項）

- Q: F2 整體範圍要不要一次交付？ → A: **拆 F2.1 + F2.2**。F2.1 = schema + transaction 紀律 + INSERT/UPDATE audit + 統一 audit path；F2.2 = outbox 模式 + Redis subscriber TTL fallback。理由：rev1 階段所有代碼都在單服務 rust process 內、未起 nestjs container、Casbin pub-sub 尚未跨微服務、outbox 現階段為 premature investment；F5 (login) 只需 F2.1 內容即可解鎖。
- Q: 既有 HTTP middleware audit 路徑 vs F3 service-level `write_in_txn` 兩條並存怎麼辦？ → A: **統一為單一 audit context API**。新增 `AuditEvent` struct + `AuditSource` enum（HTTP / Internal / Cleanup）。HTTP middleware 改走同一 helper、`source=Http`；F3 既有 helper 接 `AuditEvent`、F3 既有 `method="INTERNAL"` sentinel hack 退役（由 `AuditSource::Internal` 取代）。
...
admin 對 rust 後端的任何 write 操作（INSERT 新 entity / UPDATE 既有 entity / SOFT_DELETE 軟刪 / RESTORE 還原）自此後**永遠**寫 `sys_operation_log` 一筆 row、與業務變動同一 DB transaction commit / rollback、`payload_before` / `payload_after` 含完整 entity snapshot、敏感欄位（`password` / `access_key_secret`）自動替換為 `"<redacted>"`。整個 rust codebase 內、service / handler 層**不可能**寫業務變動而漏寫 audit — 由 Rust 類型系統（service signature 必收 `&Actor`）+ CI lint（grep service 內 INSERT/UPDATE callsite 周邊必有 audit 呼叫）+ trait-based redaction（編譯時保證敏感欄位不外洩）三重 enforcement 守護。
...
5. **Given** middleware 處理 POST `/api/sys-user` 構造 `AuditEvent { source: AuditSource::Http { method: "POST", url: "/api/sys-user", ip: "10.0.0.5", user_agent: Some("...") }, operation: Insert, ... }`，**When** 寫入，**Then** sys_operation_log row：method="POST"、url + ip + user_agent 對應填充、operation="INSERT"、**entity_type="sys_user"**（URL match 7 admin pattern、per clarify Q2）；對 POST `/api/auth/login` 等 non-admin URL、entity_type 為 fallback `"http_event"`
6. **Given** F12 cleanup-job 構造 `AuditEvent { source: AuditSource::Cleanup, operation: HardDelete, actor: Actor::system("cleanup_job"), ... }`，**When** 寫入（F12 階段、F2.1 預留 enum 值），**Then** sys_operation_log row：method="CLEANUP"、operation="HARD_DELETE"、user_id="cleanup_job"
...
#### Dimension D：Service INSERT/UPDATE handler 全 migrate
...
- **HTTP middleware 與 service-level audit 雙寫 row**（per clarify 2026-05-14 Q1）：同一 admin HTTP write **MUST** 留 2 row — 1 個 HTTP 視角（method=POST/PUT/PATCH/DELETE 等）+ 1 個 service-level（method=INTERNAL）。middleware **不**做 dedupe 檢測；forensic 兩個視角都有獨立價值。Reader 透過 method 欄區分視角、不需依賴 timestamp 或 row 順序
...
- **Migration `datas/*` seeding INSERT 是 audit-exempt**（per clarify 2026-05-14 Q3）：`rust-api/migration/src/datas/` 內 5 個 data migration（`insert_sys_user` / `insert_sys_role` / `insert_sys_menu` / `insert_sys_domain` / `insert_sys_user_role`）的 INSERT **不**走 audit_log::write_in_txn；seeding 透過 git tracked migration 檔案留紀錄即足夠。F2.1 audit 範圍限縮 application runtime（service handler + HTTP middleware）
- **sys_endpoint::sync_endpoints batch audit 策略**（per analyze 2026-05-14 M1）：sys_endpoint sync job 內部 batch upsert（INSERT 新 endpoint + UPDATE 既有變動）**MUST** 採 **N row** 策略 — 每筆 endpoint 變動寫 1 個對應 audit row、operation=Insert 或 Update、actor=`Actor::system("endpoint_sync")`、source=`Internal`。**不**採 batch-summary（1 row 概要 N 變動）。理由：(a) admin-heavy 低 throughput 場景 N row 寫入量可接受、與 Constitution §II 2N 寫入量 rationale 一致；(b) per-entity forensic 完整、reader 可 grep `entity_id` 看具體變動；(c) 與既有 service handler N row 模式一致、不引入特殊 mode
...
- **FR-006**：F3 既有 `AuditLogCtx` MUST 標 `#[deprecated]` + 提供 `From<&AuditEvent> for AuditLogCtx` 過渡層；F2.1 內所有 F3 callsite 改用 `AuditEvent`；F2.1 完成後 `AuditLogCtx` 0 active callsite
...
- **FR-015**：HTTP method → `AuditOperation` 推導規則：POST → Insert / PUT|PATCH → Update / DELETE → SoftDelete（rust 全軟刪、per F3）；GET / OPTIONS / HEAD 不寫 audit（read path）。entity_type **MUST** 走 Hybrid rule（per clarify 2026-05-14 Q2）：URL 對應 7 條 admin pattern `/api/sys-(user|role|menu|domain|organization|endpoint|access-key)/*` 任一 match → entity_type=`sys_<x>`（kebab → snake、如 `sys-access-key` → `sys_access_key`）；不 match → fallback sentinel `"http_event"`
...
- **SC-006**：admin user CRUD（create / update / delete）每個 HTTP call 在 sys_operation_log 留 **2 row**（1 HTTP 視角 method=POST/PUT/DELETE + 1 service-level method=INTERNAL、per clarify Q1）；service-level row 含 operation enum 正確（INSERT / UPDATE / SOFT_DELETE）、entity_id 對應、payload_before/after 含完整 snapshot — 端到端 quickstart 驗
...
- **SC-010**：`cargo check` 全 workspace pass（無 `-D warnings` unused-imports / dead_code）
