...
- **FR-011**：本 sprint 完成後 `docs/INTEGRATION-CHECKLIST.md` MUST：(a) 048-N1 衍生 follow-up row 從 table 移除（或標記「已結案 (a)+(b)+(c)、(d) trigger-driven 留」、保留歷史脈絡）；(b) 已完成里程碑加 049 entry；(c) Current Focus「現狀」加 049 + 「下一步」改為條件觸發 follow-up backlog 第 4 階段 active items（042-N4 / 042-N5、不再含 048-N1 因已半結案）；(d) CLAUDE.md SPECKIT marker idle。
...
- **SC-007**：完成後 `docs/INTEGRATION-CHECKLIST.md` 048-N1 衍生 follow-up row 從 table 移除（或標記「已結案 (a)+(b)+(c)」格式）+ 049 entry 加進已完成里程碑 + Current Focus 「下一步」update（不再含 048-N1）+ CLAUDE.md SPECKIT marker idle。
