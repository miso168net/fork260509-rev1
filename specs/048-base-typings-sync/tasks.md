...
- [ ] T019 outer feature branch commit 3 — INTEGRATION-CHECKLIST inline note refresh + 048 entry + Current Focus update + CLAUDE.md SPECKIT marker idle（per [data-model §E5](./data-model.md) + [quickstart Step 4.2](./quickstart.md)）：改 `docs/INTEGRATION-CHECKLIST.md` —— 衍生 follow-up `base-web TS id 型別債` inline note refresh 為「已分階段完成」格式（保留 030-034 / 039 / 040 / 048 三階段歷史）、已完成里程碑加 048 entry（outer/merge/base-web SHA placeholder 留 backfill）、Current Focus「現狀」加 048 + 「下一步」改向條件觸發 follow-up backlog 順序第 2 段；改 `CLAUDE.md` SPECKIT marker idle（Active Spec/Plan = `—`、Phase idle、下一步指向條件觸發）。`git add docs/INTEGRATION-CHECKLIST.md CLAUDE.md && git commit`。Depends on T018。對應 FR-013、SC-008。
- [ ] T020 outer feature branch commit 4 — DESIGN-W-TYPING-ALIGN §4.1 加 048 sprint 落地紀錄（per [data-model §E3 §4.1](./data-model.md) + [quickstart Step 4.3](./quickstart.md)）：將 T002 建檔時的 §4.1 placeholder（含 `<TBD post-merge>` commit SHA）保持為 placeholder（Step 6 post-merge 才回填真 SHA、本 task 只確認 §4.1 條目結構 + scope 描述完整）。`git add docs/INTEGRATION-DESIGN-W-TYPING-ALIGN.md && git commit`。**Note**：若 T002 建檔時 §4.1 已包含完整 scope 描述 + placeholder，本 task 可能為 noop commit（無 diff、skip）；若 T002 建檔時 §4.1 較簡略、本 task 補完。Depends on T019。對應 FR-009 配套。
...
| T024 (push outer feature branch) | T022、**user 同意**|
