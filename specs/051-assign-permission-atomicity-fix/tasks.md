...
- [ ] T018 C-V8 (final) full acceptance summary + git status clean：跑全 C-V1~C-V7 final run + `git status --short` clean + outer commit chain `git log --oneline 92c919a..HEAD` 含預期 commits + rust-api 1 commit + base-web 0 commit。Depends on T016 + T017 + T010 + T011 + T012 + T013 + T014。對應 SC-001~009 全 PASS、SC-009 Constitution Check 5/5 maintain。
...
| T009 (docker rebuild + restart) | T008 |
