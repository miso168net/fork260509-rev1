...
- **實作期偏離（2026-05-22，code review 發現、user 核准）**: 上述「本 feature 不改 native 行為」有一例外 —— native `assign_routes` 的 `if routes.is_empty()` guard 在 `route_ids` 為空陣列時回 `RoutesNotFound` 400，與 spec C-V5 / Edge E-5 / FR-004「空 `menuIds` 全清角色菜單授權」直接衝突（plan 的純 thin passthrough 無法滿足）。修正：guard 改為 `if !route_ids.is_empty() && routes.is_empty()` —— 空清單成為合法 clear-all（走既有 delta 全刪路徑）。此偏離經 user 核准（[AskUserQuestion 選項 A]）；偏離窄：僅改該一行 predicate，未動其他 native 邏輯。對既有 native caller 零影響 —— native `POST /authorization/assign-routes` 的 `AssignRouteDto.route_ids` 有 `#[validate(length(min = 1))]`，空清單在 `ValidatedForm` extractor 即被擋下、永不觸及該 guard，故此修正等於修一個從未被觸發的 dead-path bug。
